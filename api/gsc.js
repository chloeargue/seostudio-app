export default async function handler(req, res) {
  ...
  // Récupérer les mots-clés et positions GSC
  if (action === "keywords") {
    try {
  ...
  // Récupérer les pages les plus performantes GSC
  if (action === "pages") {
    try {
      const response = await fetch(
        `https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(site_url)}/searchAnalytics/query`,
  ...
  // Récupérer la liste des propriétés Google Analytics 4
  if (action === "ga_properties") {
    try {
      ...
      return res.status(500).json({ error: "Erreur récupération propriétés GA4" });
    }
  }

  // Récupérer les données Analytics GA4
  if (action === "analytics") {
    try {
      const response = await fetch(
      ...
      return res.status(500).json({ error: "Erreur récupération Analytics" });
    }
  }
  ...
  if (action === "analytics_pages") {
    try {
      ...
      return res.status(500).json({ error: "Erreur récupération pages Analytics" });
    }
  }
