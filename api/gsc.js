export default async function handler(req, res) {
  ...
  // Récupérer les vraies questions Google ("People Also Ask") via Serper
  if (action === "paa") {
    try {
      ...
      const questions = (data.peopleAlsoAsk || []).map(q => ({
        question: q.question,
        snippet: q.snippet || "",
      }));
      return res.status(200).json({ questions });
    } catch (err) {
      return res.status(500).json({ error: "Erreur PAA: " + err.message });
    }
  }
