  ...
  if (isArticle) {
    const callM = async (p) => {
      const r = await fetch("https://api.mistral.ai/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "mistral-small-latest", messages: [{ role: "user", content: p }], max_tokens: 1500 }),
      });
      const d = await r.json();
      return d.choices?.[0]?.message?.content || "";
    };

    const base = prompt.split("STRUCTURE OBLIGATOIRE")[0];
    
    const p1 = base + `\nRédige UNIQUEMENT cette partie (environ 600 mots):\n- Le H1 (max 170 car, contient le mot-clé)\n- META TITLE, META DESCRIPTION, URL SLUG\n- L'introduction (150 mots, commence par le mot-clé)\n- H2 section 1 avec paragraphe de 150 mots\n- H2 section 2 avec paragraphe de 150 mots\nN'utilise PAS de markdown **, *, ---.`;
    
    const p2 = base + `\nRédige UNIQUEMENT cette partie (environ 600 mots):\n- H2 section 3 avec paragraphe de 150 mots\n- H2 section 4 avec paragraphe de 150 mots\n- H2 section 5 avec paragraphe de 150 mots\nN'utilise PAS de markdown **, *, ---.`;
    
    const p3 = base + `\nRédige UNIQUEMENT cette partie finale:\n- FAQ avec 5 questions-réponses (questions comme les internautes les tapent, réponses de 60 mots)\n- Conclusion avec CTA (150 mots)\n- MAILLAGE INTERNE: 3 suggestions\n- MAILLAGE EXTERNE: 3 sources expertes\n- SYNONYMES: 10 variantes du mot-clé\n- IMAGES PEXELS: 3 requêtes en anglais\nN'utilise PAS de markdown **, *, ---.`;

    const [t1, t2, t3] = await Promise.all([callM(p1), callM(p2), callM(p3)]);
    return t1 + "\n\n" + t2 + "\n\n" + t3;
  }
...
export default function App() {
  ...
  const logout = () => { setUser(null); setClient(null); setGenResult(null); setAuditResult(null); setTracking([]); setSeasons([]); };

  if (!user) return <AuthScreen onLogin={setUser} />;
  if (!client) return <SetupScreen onSave={c => { setClient(c); setAuditSector(c.sector); setAuditUrl(c.website||""); }} user={user} onLogout={logout} />;
  ...
  const generateContent = async () => {
    ...
    const prompts = {
      article: `Tu es un expert SEO français et rédacteur web professionnel. 
Client: ${client.name} | Secteur: ${client.sector} | Ville: ${client.location} | Audience: ${client.audience||"clients locaux"} | ${sc}
Expression clé cible: "${genKw}"

Rédige un article de blog SEO expert, complet et prêt à publier sur WordPress. Article entre 2000 et 3000 mots.

RÈGLES ABSOLUES DE FORMAT:
- N'utilise JAMAIS **, *, #, ---, markdown ou labels comme "H2:", "Introduction:"
- N'écris JAMAIS d'année spécifique — utilise "actuellement", "aujourd'hui", "de nos jours"
- Phrases courtes (max 20 mots) pour faciliter la lecture et le référencement Google
- Écris directement le contenu final, prêt à copier-coller dans WordPress

STRUCTURE OBLIGATOIRE:

[H1 — max 170 caractères — contient l'expression clé "${genKw}" — accrocheur]

META TITLE: [max 60 caractères avec expression clé]
META DESCRIPTION: [max 155 caractères avec expression clé et appel à l'action]
URL SLUG: [slug-optimise-pour-le-mot-cle]

[Introduction de 150-200 mots — commence OBLIGATOIREMENT par l'expression clé "${genKw}" — explique ce que le lecteur va apprendre — donne envie de lire la suite]

[H2 — max 70 caractères — contient une variante de "${genKw}"]
[Paragraphe de 200-250 mots avec phrases courtes — inclut des données chiffrées réalistes — inclut des synonymes de l'expression clé]

[H3 — max 70 caractères — sous-thème du H2]
[Paragraphe de 150-200 mots avec exemples concrets]

[H2 — max 70 caractères — contient une variante de "${genKw}"]
[Paragraphe de 200-250 mots — conseil pratique et actionnable]

[H3 — max 70 caractères]
[Paragraphe de 150-200 mots]

[H2 — max 70 caractères — contient une variante de "${genKw}"]
[Paragraphe de 200-250 mots]

FAQ — Questions fréquentes (optimisé pour la recherche IA/GEO):
[Question 1 exactement comme les internautes la tapent sur Google ?]
[Réponse courte et directe de 50-80 mots]
...
[Conclusion de 150 mots avec récapitulatif et appel à l'action clair pour contacter ${client.name}]

MAILLAGE INTERNE SUGGÉRÉ: [3 suggestions de liens internes vers d'autres pages du site]
MAILLAGE EXTERNE SUGGÉRÉ: [3 sources expertes et autoritaires à citer]
SYNONYMES ET CHAMP SÉMANTIQUE: [10 variantes et synonymes de l'expression clé à intégrer]
IMAGES PEXELS SUGGÉRÉES: [3 requêtes de recherche en anglais pour trouver des images sur pexels.com]`,
    ...
    try { const r = await callMistral(prompts[genType]); setGenResult(r); } catch { setGenResult("❌ Erreur de génération."); } finally { setGenerating(false); }
  };
          ...
          {page === "dashboard" && (
            <div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:"1.5rem" }}>
                {[
                  { l:"Sessions ce mois", v:tracking.length?tracking[tracking.length-1].sessions.toLocaleString():"—", c:"#16a34a" },
                  { l:"Mots-clés positionnés", v:tracking.length?tracking[tracking.length-1].keywords:"—", c:"#2563eb" },
                  { l:"Articles publiés", v:tracking.length?tracking[tracking.length-1].articles:"—", c:"#d97706" },
                  { l:"Conversions", v:tracking.length?tracking[tracking.length-1].conversions:"—", c:"#dc2626" },
                ].map(({ l, v, c }) => (
              ...
              {!tracking.length && <div style={{ background:"#fff", borderRadius:12, border:"1px solid #e5e7eb", padding:"2rem", textAlign:"center", marginBottom:"1.5rem" }}>
                <p style={{ fontSize:32 }}>📊</p>
                <p style={{ fontSize:14, fontWeight:600, marginBottom:4 }}>Aucune donnée encore</p>
                <p style={{ fontSize:13, color:"#6b7280", marginBottom:16 }}>Ajoute les données du premier mois dans Suivi mensuel</p>
                <button onClick={() => setPage("tracking")} style={{ fontSize:13, color:"#fff", background:"#16a34a", border:"none", borderRadius:8, padding:"9px 18px", cursor:"pointer", fontWeight:600 }}>Ajouter les données →</button>
              </div>}
              {tracking.length > 0 && <div style={{ background:"#fff", borderRadius:12, padding:"1.25rem", border:"1px solid #e5e7eb", marginBottom:"1.5rem" }}>
                <p style={{ fontSize:13, fontWeight:600, marginBottom:12 }}>Progression sessions</p>
                <div style={{ display:"flex", alignItems:"flex-end", gap:8, height:100 }}>
                  {tracking.map((m,i) => <div key={m.id} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}><div style={{ width:"100%", background:i===tracking.length-1?"#16a34a":"#bbf7d0", borderRadius:"4px 4px 0 0", height:`${(m.sessions/Math.max(...tracking.map(x=>x.sessions)))*90}px`, minHeight:4 }} /><span style={{ fontSize:9, color:"#9ca3af" }}>{m.month}</span></div>)}
                </div>
              </div>}
