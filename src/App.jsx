...
export default function App() {
  ...
  const optimizePage = async (page) => {
    ...
    try {
      ...
      // 3. Générer métas + optimisation contenu avec Mistral
      const prompt = `Tu es un expert SEO français.
Site: ${client.name} (${client.sector}, ${client.location})
Page: "${pageTitle}"
URL: /${page.slug}
Contenu actuel (extrait): ${pageContent.slice(0, 500)}
Mots-clés GSC du site: ${topKw || client.sector}

Génère UNIQUEMENT ce JSON valide sans backticks ni markdown:
{
  "meta_title": "[titre SEO max 60 car, contient le mot-clé principal]",
  "meta_description": "[description 155 car max avec CTA]",
  "focus_keyword": "[mot-clé principal de cette page]",
  "h1_optimise": "[nouveau H1 optimisé max 70 car]",
  "h2_suggestions": ["H2 suggestion 1", "H2 suggestion 2", "H2 suggestion 3"],
  "intro_optimisee": "[paragraphe d'introduction optimisé de 100 mots, commence par le mot-clé]",
  "recommandations": ["Recommandation SEO 1", "Recommandation SEO 2", "Recommandation SEO 3"]
}`;
      ...
      const d = await r.json();
      const meta = JSON.parse(d.text.replace(/```json|```/g, '').trim());
