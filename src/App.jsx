...
{"titre":"[H1 accrocheur max 70 car contenant ${blogKw}]","slug":"[slug-optimise-avec-mot-cle]","meta_title":"[EXACTEMENT max 60 car avec ${blogKw}]","meta_description":"[EXACTEMENT max 150 car avec ${blogKw} et CTA]","extrait":"[résumé 25 mots]","contenu_html":"[HTML complet: intro avec mot-clé, H2 avec variantes, paragraphes, liens internes/externes, FAQ en H3, conclusion CTA]"}`;
  ...
  const searchKeywords = async () => {
    ...
    try {
      // Chercher via Serper les mots-clés populaires
      const queries = [
        `${client.sector} ${client.location}`,
        `meilleur ${client.sector} ${client.location}`,
        `comment ${client.sector.toLowerCase()}`,
      ];
      ...
      const results = await Promise.all(queries.map(q =>
        fetch('/api/generate', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            prompt: `Donne-moi 8 mots-clés SEO très recherchés sur Google pour une entreprise "${client.name}" dans le secteur "${client.sector}" à "${client.location}". Réponds UNIQUEMENT avec ce JSON sans backticks: {"keywords":[{"kw":"mot-clé","intent":"informationnelle|transactionnelle|navigationnelle","difficulte":"facile|moyenne|difficile"},...]}`
          })
        }).then(r => r.json())
      ));

      // Recherche Serper pour données réelles
      const serperRes = await fetch('/api/gsc', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'serper_keywords',
          query: `${client.sector} ${client.location}`,
          access_token: gscToken
        })
      }).then(r => r.json()).catch(() => ({}));

      // Parser les suggestions IA
      const aiKws = [];
      results.forEach(r => {
        try {
          const parsed = JSON.parse(r.text.replace(/```json|```/g, '').trim());
          if (parsed.keywords) aiKws.push(...parsed.keywords);
        } catch(e) {}
      });

      // Dédupliquer
      const seen = new Set();
      const unique = aiKws.filter(k => {
        if (seen.has(k.kw)) return false;
        seen.add(k.kw);
        return true;
      });

      // Ajouter les suggestions GSC existantes
      const gscKws = gscKeywords.slice(0,5).map(k => ({
        kw: k.keys?.[0],
        intent: 'navigationnelle',
        difficulte: k.position <= 10 ? 'facile' : 'moyenne',
        source: 'GSC',
        position: Math.round(k.position),
        clics: k.clicks
      })).filter(k => k.kw);

      setKwSuggestions([...gscKws, ...unique.slice(0, 15)]);
    } catch(e) {
                  ...
                  <div>
                    ...
                    <div style={{ maxHeight:200, overflowY:"auto", display:"flex", flexDirection:"column", gap:3 }}>
                      {kwSuggestions.map((k,i) => (
                        <button key={i} onClick={() => setBlogKw(k.kw)} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", width:"100%", textAlign:"left", fontSize:11, padding:"6px 8px", borderRadius:6, border:"1px solid #e5e7eb", background: blogKw===k.kw?"#f0fdf4":"#f9fafb", cursor:"pointer" }}>
                          <span>+ {k.kw}</span>
