...
export default function App() {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('seostudio_user');
    if (savedUser) {
      const u = JSON.parse(savedUser);
      setUser(u);
      const savedClient = localStorage.getItem('seostudio_client_' + u.id);
      if (savedClient) {
  ...
  const logout = () => { if(user) { localStorage.removeItem('seostudio_client_' + user.id); localStorage.removeItem('seostudio_user'); } setUser(null); setClient(null); setGenResult(null); setAuditResult(null); setTracking([]); setSeasons([]); };
