...
{"keywords":[
  ...
  const optimizeDiviContent = async (page) => {
    ...
    try {
      ...
      const texts = modules.filter(m => m.type === 'text').map(m => m.content).slice(0, 4);
...
Contenu actuel: ${texts.join(' | ').slice(0, 800)}
Mots-clés GSC: ${topKw || client.sector}
Nombre de sections texte: ${texts.length}
Nombre de FAQ: ${faqs.length}

MISSION: Réécris uniquement les textes de cette page en version optimisée SEO.
RÈGLES STRICTES:
- Garde EXACTEMENT le même nombre de sections (${texts.length}) et de FAQ (${faqs.length})
...
- Garde le même ton professionnel et le même secteur d'activité

Génère UNIQUEMENT ce JSON valide sans backticks:
{
  "h1": "nouveau H1 optimisé avec mot-clé",
  "sections": ${JSON.stringify(texts.map((_, i) => ({ h2: `Titre section ${i+1}`, texte: `Contenu section ${i+1}` })))},
  "faq": ${JSON.stringify(faqs.map(f => ({ question: f.question, reponse: f.answer })))}
}`;
