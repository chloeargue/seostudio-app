...
const callMistral = async (prompt) => {
  ...
  if (isArticle) {
    // 2 appels parallèles pour contourner la limite de tokens
    const prompt1 = prompt + "\n\nGénère UNIQUEMENT : le H1, les métas, l'introduction et les 3 premières sections H2 avec leur contenu complet.";
    const prompt2 = prompt + "\n\nGénère UNIQUEMENT la suite de l'article : 4ème et 5ème section H2, la FAQ avec 5 questions-réponses optimisées GEO, la conclusion avec CTA, le maillage interne, le maillage externe, les synonymes et les suggestions d'images Pexels.";
    
    const [r1, r2] = await Promise.all([
      fetch("https://api.mistral.ai/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${MISTRAL_KEY}` },
        body: JSON.stringify({ model: "mistral-small-latest", messages: [{ role: "user", content: prompt1 }], max_tokens: 3000 }),
      }),
      fetch("https://api.mistral.ai/v1/chat/completions", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${MISTRAL_KEY}` },
        body: JSON.stringify({ model: "mistral-small-latest", messages: [{ role: "user", content: prompt2 }], max_tokens: 3000 }),
      })
    ]);
    
    const d1 = await r1.json();
    const d2 = await r2.json();
    return (d1.choices?.[0]?.message?.content || "") + "\n\n" + (d2.choices?.[0]?.message?.content || "");
  }
