            ...
            <div>
              ...
              {gscToken && (
                <div style={{ background:"#fff", borderRadius:12, border:"1px solid #e5e7eb", padding:"1.25rem", marginBottom:"1.5rem" }}>
                  ...
                  {!gaData && !gaLoading && !gaProperty && (
                    <div>
                      <p style={{ fontSize:12, color:"#4b5563", marginBottom:4 }}>Entre ton ID de propriété GA4 :</p>
                      <p style={{ fontSize:11, color:"#9ca3af", marginBottom:8 }}>Analytics → Admin → Paramètres propriété (ex: 539896426)</p>
                      <div style={{ display:"flex", gap:8 }}>
                        <input id="ga-input" placeholder="Ex: 539896426" style={{ flex:1, fontSize:12, padding:"7px 8px", borderRadius:8, border:"1px solid #d1d5db" }} />
                        <button onClick={() => { const v = document.getElementById('ga-input').value.trim(); if(!v) return; setGaProperty(v); localStorage.setItem('seos_ga_property', v); loadGAData(gscToken, v); }} style={{ fontSize:12, color:"#fff", background:"#16a34a", border:"none", borderRadius:8, padding:"7px 14px", cursor:"pointer", fontWeight:600 }}>Connecter</button>
                      </div>
                    </div>
                  )}
                  {gaData && !gaLoading && (
                    <div>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                        ...
                        <button onClick={() => { setGaProperty(null); localStorage.removeItem('seos_ga_property'); setGaData(null); setGaPages([]); }} style={{ fontSize:10, color:"#6b7280", background:"none", border:"none", cursor:"pointer", textDecoration:"underline" }}>Changer</button>
                      </div>
                      ...
                      {gaPages.length > 0 && (
                        <div>
                          <p style={{ fontSize:11, fontWeight:600, marginBottom:8 }}>Top pages</p>
                          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                            ...
                            <tbody>{gaPages.slice(0,5).map((p,i) => <tr key={i} style={{ borderBottom:"1px solid #f3f4f6" }}><td style={{ padding:"5px 8px", maxWidth:180, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{p.path}</td><td style={{ padding:"5px 8px", color:"#16a34a", fontWeight:600 }}>{p.sessions}</td><td style={{ padding:"5px 8px", color:p.bounceRate>70?"#dc2626":"#16a34a" }}>{p.bounceRate}%</td><td style={{ padding:"5px 8px", color:"#6b7280" }}>{Math.floor(p.duration/60)}m{p.duration%60}s</td></tr>)}</tbody>
                          </table>
                        </div>
                      )}
                    </div>
