...
{"keywords":[
  ...
  const optimizeDiviContent = async (page) => {
    ...
    try {
      ...
      // 1. Lire le contenu Divi via le plugin SEO Studio Connector v2
      const readRes = await fetch(`${base}/wp-json/seostudio/v1/divi/read/${page.id}`, {
        headers: { 'Authorization': `Basic ${creds}` }
      });
      const diviData = await readRes.json();
      if (!readRes.ok) throw new Error(diviData.message || 'Erreur lecture Divi');
...
Génère UNIQUEMENT ce JSON valide sans backticks:
{
  ...
  const publishDiviContent = async () => {
    ...
    try {
      ...
      // Utiliser l'endpoint Divi du plugin pour mettre à jour proprement
      const res = await fetch(`${base}/wp-json/seostudio/v1/divi/update/${diviPreview.page.id}`, {
        method: 'POST',
        headers: { 'Authorization': `Basic ${creds}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
