            ...
            <div>
              ...
              {gscToken && (
                <div style={{ background:"#fff", borderRadius:12, border:"1px solid #e5e7eb", padding:"1.25rem", marginBottom:"1.5rem" }}>
                  ...
                  {!gaData && !gaLoading && gaProperties.length > 0 && !gaProperty && (
                    <div>
                      <p style={{ fontSize:12, color:"#4b5563", marginBottom:8 }}>Sélectionne ta propriété GA4 :</p>
                      <select onChange={e => { const p = e.target.value; if(!p) return; setGaProperty(p); localStorage.setItem('seos_ga_property', p); loadGAData(gscToken, p); }} style={{ width:"100%", fontSize:12, padding:"7px 8px", borderRadius:8, border:"1px solid #d1d5db" }}>
                        <option value="">-- Choisir --</option>
                        {gaProperties.map(p => <option key={p.id} value={p.id}>{p.name} ({p.account})</option>)}
                      </select>
                    </div>
                  )}
                  {!gaData && !gaLoading && gaProperties.length === 0 && <p style={{ fontSize:12, color:"#9ca3af" }}>Aucune propriété GA4 trouvée sur ce compte.</p>}
                  {gaData && !gaLoading && (
                    <div>
