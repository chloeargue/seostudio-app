  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f9fafb", display:"flex", alignItems:"center", justifyContent:"center", padding:"2rem" }}>
      <div style={{ background:"#fff", borderRadius:16, border:"1px solid #e5e7eb", padding:"2.5rem", width:"100%", maxWidth:480 }}>
        ...
        <p style={{ fontSize:14, color:"#6b7280", marginBottom:"2rem" }}>Configure le profil client pour personnaliser tout le contenu SEO.</p>
        {[
          { key:"name", label:"Nom de l'entreprise", placeholder:"Ex: Plomberie Dupont", required:true },
          { key:"sector", label:"Secteur d'activité", placeholder:"Ex: Plomberie, Restauration, Immobilier...", required:true },
          { key:"location", label:"Zone géographique", placeholder:"Ex: Lyon, Paris, Bordeaux...", required:true },
          { key:"website", label:"Site web (optionnel)", placeholder:"https://www.monsite.fr" },
          { key:"audience", label:"Cible client (optionnel)", placeholder:"Ex: Particuliers, PME, B2B..." },
...
export default function App() {
  ...
  if (!client) return <SetupScreen onSave={c => { setClient(c); setAuditSector(c.sector); setAuditUrl(c.website||""); }} />;
  ...
  const generateContent = async () => {
    ...
    const prompts = {
      article: `Tu es un expert SEO français. Nous sommes en 2026. Client: ${client.name} (${client.sector}, ${client.location}). Audience: ${client.audience||"clients locaux"}. ${sc} Mot-clé cible: "${genKw}". Génère un article de blog SEO complet. IMPORTANT: N'utilise JAMAIS de **, *, --- ou tout autre formatage markdown. Écris du texte brut uniquement. Structure:\nTITRE H1: [titre optimisé]\nMETA TITLE: [60 car max]\nMETA DESCRIPTION: [155 car max]\nINTRODUCTION: [2 phrases percutantes]\nH2 PRINCIPAL: [titre section]\n[80 mots de contenu]\nH2 SECONDAIRE: [titre section]\n[80 mots de contenu]\nCONCLUSION: [CTA clair]\nMOTS-CLÉS SECONDAIRES: [liste 5 mots-clés]\nBALISES HN SUGGÉRÉES: [liste H2/H3]`,
      meta: `Tu es un expert SEO français. Nous sommes en 2026. Client: ${client.name} (${client.sector}). ${sc} Mot-clé: "${genKw}". IMPORTANT: N'utilise JAMAIS de **, *, --- ou markdown. Texte brut uniquement. Génère:\nMETA TITLE (60 car max): [titre]\nMETA DESCRIPTION (155 car max): [desc]\nH1: [titre page]\nH1 ALTERNATIF: [variante]\nURL SLUG: [slug]\nMOTS-CLÉS LSI: [5 variantes sémantiques]`,
      keywords: `Tu es un expert SEO français. Nous sommes en 2026. Client: ${client.name} (${client.sector}, ${client.location}). ${sc} Thème: "${genKw}". IMPORTANT: N'utilise JAMAIS de **, *, --- ou markdown. Texte brut uniquement. Génère:\nMOTS-CLÉS PRINCIPAUX (fort volume): liste 6\nLONGUE TRAÎNE (intention précise): liste 8\nQUESTIONS FAQ (featured snippets): liste 5\nMOTS-CLÉS LOCAUX: liste 4\nMOTS-CLÉS SAISONNIERS: liste 4`,
    };
    try {
                ...
                {genResult && !generating && <div>
                  ...
                  <div style={{ fontSize:13, lineHeight:1.8, color:"#374151" }}>
                    {genResult.split("\n").map((line,i) => {
                      ...
                      if (/^(META|BALISE|MOT|URL|H1|MOTS)/.test(line)) return <div key={i} style={{ background:"#f9fafb", borderRadius:6, padding:"6px 10px", marginBottom:5, fontSize:12, fontFamily:"monospace", border:"1px solid #e5e7eb" }}>{line}</div>;
          ...
          {page === "tracking" && (
            <div>
              ...
              {!tracking.length && !showAddMonth ? <div style={{ background:"#fff", borderRadius:12, border:"1px solid #e5e7eb", padding:"2.5rem", textAlign:"center" }}>
                <p style={{ fontSize:32, marginBottom:8 }}>📈</p>
                <p style={{ fontSize:14, fontWeight:600, marginBottom:4 }}>Aucune donnée encore</p>
                <p style={{ fontSize:13, color:"#6b7280", marginBottom:16 }}>Ajoute les données du premier mois pour démarrer le suivi</p>
