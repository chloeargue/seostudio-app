...
export default function App() {
  ...
  const [gaProperty, setGaProperty] = useState(null);
    ...
    try {
      ...
      if (savedToken) {
        ...
        if (savedSite) {
          ...
          // Charger automatiquement les données GSC
          setTimeout(() => loadGSCData(savedToken, savedSite), 500);
        } else {
          // Charger la liste des sites automatiquement
  ...
  const loadGAProperties = async (token) => {
    if (!token) return;
    try {
      const data = await fetch('/api/gsc', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'ga_properties', access_token:token }) }).then(r=>r.json());
      if (data.properties) setGaProperties(data.properties);
    } catch(e) { console.error('GA properties error:', e); }
  };

  const loadGAData = async (token, propId) => {
    if (!token || !propId) return;
    setGaLoading(true);
    try {
      const [analytics, pages] = await Promise.all([
        fetch('/api/gsc', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'analytics', access_token:token, property_id:propId }) }).then(r=>r.json()),
        fetch('/api/gsc', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'analytics_pages', access_token:token, property_id:propId }) }).then(r=>r.json()),
      ]);
      if (analytics.sessions !== undefined) setGaData(analytics);
      if (pages.pages) setGaPages(pages.pages);
    } catch(e) { console.error('GA data error:', e); }
    setGaLoading(false);
  };
  ...
  const disconnectGSC = () => {
    ...
    localStorage.removeItem('seos_ga_property');
    setGscToken(null); setGscSite(null); setGscKeywords([]); setGscPages([]);
    setGaData(null); setGaPages([]); setGaProperty(null); setGaProperties([]);
  };
          ...
          {page === "dashboard" && (
              ...
              {gscToken && (
                <div style={{ background:"#fff", borderRadius:12, border:"1px solid #e5e7eb", padding:"1.25rem", marginBottom:"1.5rem" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                    <span style={{ fontSize:18 }}>📊</span>
                    <p style={{ fontSize:13, fontWeight:600, margin:0 }}>Google Analytics 4</p>
                    {gaLoading && <span style={{ fontSize:11, color:"#9ca3af" }}>⏳ Chargement...</span>}
                  </div>
                  {gaProperties.length > 0 && !gaProperty && (
                    <div>
                      <p style={{ fontSize:12, color:"#4b5563", marginBottom:8 }}>Sélectionne ta propriété GA4 :</p>
                      <select onChange={e => { const p = e.target.value; setGaProperty(p); localStorage.setItem('seos_ga_property', p); loadGAData(gscToken, p); }} style={{ width:"100%", fontSize:12, padding:"7px 8px", borderRadius:8, border:"1px solid #d1d5db" }}>
                        <option value="">-- Choisir --</option>
                        {gaProperties.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                      </select>
                    </div>
                  )}
                  {gaData && !gaLoading && (
                    <div>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                        <span style={{ fontSize:11, color:"#6b7280" }}>28 derniers jours</span>
                        <button onClick={() => { setGaProperty(null); localStorage.removeItem('seos_ga_property'); setGaData(null); setGaPages([]); loadGAProperties(gscToken); }} style={{ fontSize:10, color:"#6b7280", background:"none", border:"none", cursor:"pointer", textDecoration:"underline" }}>Changer</button>
                      </div>
                      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:12 }}>
                        {[
                          { l:"Sessions", v:gaData.sessions?.toLocaleString(), c:"#16a34a" },
                          { l:"Utilisateurs", v:gaData.users?.toLocaleString(), c:"#2563eb" },
                          { l:"Taux de rebond", v:gaData.bounceRate+"%", c:"#d97706" },
                          { l:"Durée moy.", v:Math.floor(gaData.avgDuration/60)+"m "+gaData.avgDuration%60+"s", c:"#8b5cf6" },
                          { l:"Conversions", v:gaData.conversions?.toLocaleString(), c:"#dc2626" },
                          { l:"Pages vues", v:gaData.pageviews?.toLocaleString(), c:"#0891b2" },
                        ].map(({ l, v, c }) => (
                      ...
                      {gaPages.length > 0 && (
                        <div>
                          <p style={{ fontSize:11, fontWeight:600, color:"#374151", marginBottom:8 }}>Top pages</p>
                          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                            <thead><tr style={{ borderBottom:"1px solid #e5e7eb" }}>{["Page","Sessions","Rebond","Durée"].map(h => <th key={h} style={{ textAlign:"left", padding:"4px 8px", color:"#6b7280", fontWeight:600 }}>{h}</th>)}</tr></thead>
                            <tbody>{gaPages.slice(0,5).map((p,i) => <tr key={i} style={{ borderBottom:"1px solid #f3f4f6" }}><td style={{ padding:"5px 8px", maxWidth:200, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{p.path}</td><td style={{ padding:"5px 8px", color:"#16a34a", fontWeight:600 }}>{p.sessions}</td><td style={{ padding:"5px 8px", color:p.bounceRate>70?"#dc2626":"#16a34a" }}>{p.bounceRate}%</td><td style={{ padding:"5px 8px", color:"#6b7280" }}>{Math.floor(p.duration/60)}m{p.duration%60}s</td></tr>)}</tbody>
                          </table>
                        </div>
                      )}
                    </div>
                  )}
                  {!gaData && !gaLoading && !gaProperty && gaProperties.length === 0 && <p style={{ fontSize:12, color:"#9ca3af" }}>Aucune propriété GA4 trouvée.</p>}
                </div>
              )}
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:"1.5rem" }}>
                {[
                  ...
                  { l:"Sessions GA4", v:gaData ? gaData.sessions?.toLocaleString() : "—", c:"#2563eb" },
                  { l:"Mots-clés positionnés", v:gscKeywords.length || "—", c:"#d97706" },
