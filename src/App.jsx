...
{"keywords":[
  ...
  const optimizeDiviContent = async (page) => {
    ...
    try {
      ...
      // 1. Récupérer le contenu brut de la page
      const pageRes = await fetch(`${base}/wp-json/wp/v2/pages/${page.id}?context=edit`, {
        headers: { 'Authorization': `Basic ${creds}` }
      });
      const pageData = await pageRes.json();
      const rawContent = pageData.content?.raw || '';
      const pageTitle = pageData.title?.rendered || page.title?.rendered || '';

      // 2. Extraire les textes visibles (sans shortcodes Divi)
...
')
      ...
      // 3. Générer le contenu optimisé avec l'IA
...
Page: "${pageTitle}"
Mots-clés GSC: ${topKw || client.sector}
Contenu actuel: ${visibleText}

MISSION: Réécris uniquement les textes de cette page en version optimisée SEO.
RÈGLES STRICTES:
- Garde EXACTEMENT la même structure (nombre de sections, FAQ, boutons)
- Optimise le mot-clé principal dans les titres et le premier paragraphe
- Phrases courtes (max 20 mots)
- Min 30% de mots de transition
- Max 10% de voix passive
- Garde le même ton professionnel

Génère UNIQUEMENT ce JSON valide sans backticks:
{
  "h1": "nouveau H1 optimisé",
  "sections": [
    {"h2": "titre section 1", "texte": "contenu paragraphe section 1"},
    {"h2": "titre section 2", "texte": "contenu paragraphe section 2"},
    {"h2": "titre section 3", "texte": "contenu paragraphe section 3"},
    {"h2": "titre section 4", "texte": "contenu paragraphe section 4"}
  ],
  "faq": [
  ...
  "cta": "texte du bouton CTA"
      ...
      // 4. Reconstruire le contenu Divi en remplaçant uniquement les textes
      ...
      // Remplacer les H2 dans l'ordre
      let h2Count = 0;
      newContent = newContent.replace(
        /(<h2[^>]*>)([^<]*?)(<\/h2>)/gi,
        (match, open, text, close) => {
          if (optimized.sections[h2Count]) {
            const replacement = `${open}${optimized.sections[h2Count].h2}${close}`;
            h2Count++;
            return replacement;
          }
          return match;
      ...
      setDiviPreview({ page, optimized, newContent, originalTitle: pageTitle });
    } catch(e) {
  ...
  const publishDiviContent = async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/wp-json/wp/v2/pages/${diviPreview.page.id}`, {
        method: 'POST',
        headers: { 'Authorization': `Basic ${creds}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          title: diviPreview.optimized.h1,
          content: diviPreview.newContent,
        })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Erreur publication');
