...
export default function App() {
  ...
  const optimizePage = async (page) => {
    setWpOptimizing(page.id); setWpSuccess(null);
    try {
      const creds = btoa(`${wpUser}:${wpPass}`);
      const base = wpUrl.replace(/\/$/, '');
      
      // Récupérer les mots-clés GSC pertinents pour cette page
      const topKw = gscKeywords.slice(0,3).map(k => k.keys?.[0]).filter(Boolean).join(', ');
      
      // Générer méta optimisée avec Mistral
      const prompt = `Tu es un expert SEO français. 
Page WordPress: "${page.title?.rendered}"
Site: ${client.name} (${client.sector}, ${client.location})
Mots-clés GSC top: ${topKw || genKw || client.sector}
URL: ${base}/${page.slug}

Génère UNIQUEMENT ce JSON valide sans backticks:
{"meta_title":"[titre SEO 60 car max avec mot-clé]","meta_description":"[description 155 car max avec CTA]","focus_keyword":"[mot-clé principal]"}`;
      ...
      // Mettre à jour via l'API WordPress native (compatible Yoast + Rank Math)
      const updateRes = await fetch(`${base}/wp-json/wp/v2/pages/${page.id}`, {
        ...
        body: JSON.stringify({
          title: page.title?.rendered, // garde le titre existant
          meta: {
            // Yoast SEO
            ...
            // WordPress natif
            _wp_page_template: undefined,
          }
        })
      });

      if (!updateRes.ok) {
        // Si la mise à jour des métas échoue, on essaie via les champs SEO directs
        const updateRes2 = await fetch(`${base}/wp-json/wp/v2/pages/${page.id}`, {
          method: 'POST',
          headers: {
            'Authorization': `Basic ${creds}`,
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            ...
            excerpt: { rendered: meta.meta_description },
          })
        });
        if (!updateRes2.ok) throw new Error("Mise à jour échouée — vérifie que l'API REST est activée dans WordPress");
      }
            ...
            <div>
              {!wpConnected ? (
                ...
                <div>
                  ...
                  {wpSuccess && <div style={{ background:"#dcfce7", borderRadius:10, padding:"1rem", marginBottom:16, border:"1px solid #bbf7d0" }}>
                    <p style={{ fontSize:13, fontWeight:600, color:"#15803d", marginBottom:6 }}>✅ Page optimisée !</p>
                    <div style={{ fontSize:11, fontFamily:"monospace", color:"#166534" }}>
                      <p>META TITLE: {wpSuccess.meta.meta_title}</p>
                      <p>META DESC: {wpSuccess.meta.meta_description}</p>
                      <p>FOCUS KW: {wpSuccess.meta.focus_keyword}</p>
                    </div>
                  </div>}
                  {wpError && <div style={{ background:"#fee2e2", borderRadius:8, padding:"8px 12px", marginBottom:12, fontSize:12, color:"#991b1b" }}>{wpError}</div>}
