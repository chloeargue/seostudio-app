...
export default function App() {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('seos_user');
    if (savedUser) {
      const u = JSON.parse(savedUser);
      setUser(u);
      const savedClient = localStorage.getItem('seos_client_' + (u.email || u.id));
      if (savedClient) {
  ...
  const logout = () => { if(user) { localStorage.removeItem('seos_client_' + (user.email||user.id)); localStorage.removeItem('seos_user'); } setUser(null); setClient(null); setGenResult(null); setAuditResult(null); setTracking([]); setSeasons([]); };

  if (!user) return <AuthScreen onLogin={(u) => {
    setUser(u);
    localStorage.setItem('seos_user', JSON.stringify(u));
    const saved = localStorage.getItem('seos_client_' + (u.email || u.id));
  ...
  if (!client) return <SetupScreen onSave={c => {
    ...
    localStorage.setItem('seos_client_' + (user.email || user.id), JSON.stringify(c));
